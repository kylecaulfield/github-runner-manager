...
- **Login Items & Notifications (optional).** Launch at Login and local notifications are opt-in in Settings. Both are keyed to a stable code signature, so under the default **ad-hoc "Sign to Run Locally"** build they may not persist or deliver reliably — they become dependable once you sign with a **Developer ID** identity (see App Sandbox above). The app degrades gracefully if either is unavailable, and the in-app banner always works as a fallback.
...
- **Remove** — de-register a runner from GitHub and tear down its service. The app stops and uninstalls the LaunchAgent, then (with a PAT) mints a fresh remove token and runs `config.sh remove`, with an API `DELETE` backstop that de-registers even if `config.sh remove` fails; without a PAT it falls back to a **local-only** removal (`config.sh remove --local`), leaving the registration to be cleared from GitHub's UI. Optionally, it can also **delete the install directory** (a separate confirmation).
- **GitHub-side state** — with a PAT set, each runner also shows its **server-side** state from the API (online / offline, and a **busy** indicator when it's mid-job) next to the local launchd status — so you can tell a live-but-deregistered runner from a truly working one. Update/Remove warn when a runner is currently running a job.
- **Fleet controls** — a summary header (running / stopped / updates-available counts + "last refreshed"), a **search** box and **sort** (by status, name, or repo), and **Start All / Stop All** bulk actions. Right-click a runner for quick **Start/Stop**, **Open on GitHub** (its runners settings page), and **Copy install path / labels**. Keyboard shortcuts: ⌘R refresh, ⌘N new runner, ⌘, settings.
- **Runs at launch** — discovery, status polling, and the menu-bar health glyph start when the app launches (you don't have to open the popover first). Pair with **Launch at Login** (Settings) so the mini comes back after a reboot.
- **Notifications (opt-in)** — optional local notifications when a runner stops unexpectedly or an update becomes available (Settings → Notifications).
